...
for (const sourceFixtureId of fixtureIds) {
  for (const targetFixtureId of fixtureIds) {
    ...
    const sourceConsentedBuyers = new Set(
      sourceRows.filter((row) => row.consent_status === "consented").map((row) => row.supporter_id_hash)
    );
...
console.log(JSON.stringify({
  ...
  repeatCohorts: repeatCohorts.length,
  target: "data/live/crm-ticketing.json",
  state: "club-aggregate",
